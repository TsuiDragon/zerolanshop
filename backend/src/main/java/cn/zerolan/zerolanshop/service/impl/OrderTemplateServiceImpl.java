...
public class OrderTemplateServiceImpl implements OrderTemplateService {
    ...
    public OrderTemplateServiceImpl(
    ...
    public OrderTemplateResponse create(OrderTemplateCreateRequest request) {
        if (request == null) {
            throw new RuntimeException("订单模板请求不能为空");
        }
    ...
    public OrderTemplateResponse updateStatus(Long id, OrderTemplateStatusRequest request) {
        if (request == null) {
            throw new RuntimeException("订单模板状态请求不能为空");
        }
        OrderTemplate template = getExistingTemplate(id);
    ...
    public OrderTemplate getExistingTemplate(Long id) {
        if (id == null || id <= 0) {
            throw new RuntimeException("订单模板 ID 不能为空");
        }
    ...
    private void replaceFields(Long templateId, List<OrderTemplateFieldRequest> requests) {
        if (requests == null || requests.isEmpty()) {
            throw new RuntimeException("订单模板至少需要包含一个字段");
        }
    ...
    private void validateSameName(Long currentId, String name) {
        ...
        if (orderTemplateMapper.selectCount(wrapper) > 0) {
            throw new RuntimeException("订单模板名称已存在");
        }
    }
    ...
    private String normalizeName(String name) {
        ...
        if (!StringUtils.hasText(normalized)) {
            throw new RuntimeException("订单模板名称不能为空");
        }
        if (normalized.length() > 50) {
            throw new RuntimeException("订单模板名称不能超过 50 个字符");
        }
        return normalized;
    }
    ...
    private String normalizeFieldName(String fieldName) {
        ...
        if (!StringUtils.hasText(normalized)) {
            throw new RuntimeException("订单模板字段名称不能为空");
        }
        if (normalized.length() > 50) {
            throw new RuntimeException("订单模板字段名称不能超过 50 个字符");
        }
        return normalized;
    }
    ...
    private String normalizeFieldType(String fieldType) {
        ...
        if (!List.of(FIELD_PHONE, FIELD_QQ, FIELD_EMAIL, FIELD_ADDRESS, FIELD_TEXT).contains(normalized)) {
            throw new RuntimeException("订单模板字段类型无效");
        }
        return normalized;
    }
